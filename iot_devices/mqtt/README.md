...
uv run -m iot_devices.mqtt.client_cli

# Subscribe to specific topic
uv run -m iot_devices.mqtt.client_cli --topic "device/sensors/#"

# Publish a message
uv run -m iot_devices.mqtt.client_cli --publish "test/topic" --message "Hello World"
uv run -m iot_devices.mqtt.client_cli --publish "device/control/{CLIENT_ID}" --message "shutdown"

# Publish weather data for cities (LON, PRG, BRN)
uv run -m iot_devices.mqtt.client_cli --city LON
uv run -m iot_devices.mqtt.client_cli --city PRG
uv run -m iot_devices.mqtt.client_cli --city BRN

# Custom client ID with subscription
uv run -m iot_devices.mqtt.client_cli --client-id "my_device" --topic "device/sensors/#"
```
